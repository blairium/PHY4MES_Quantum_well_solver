...
%L =1.237195e-08 well width for 3 quantum states with energy less
%than 0.33ev
% approx 412 mesh points in width
a = 3e-11 ;% mesh point spacing
mu = 1 ;% Fermi level in eV
Temp = 293; % Kelvin
Nout = 100; %mesh points
Nin = 333; %Mesh point in inner barrier
Nw1 = 2*412; %mesh points aka size well 1
Nw2 = 2*Nw1; %mesh points aka size well 2 - twice the size of well 1 for this section
E_m = .33 ; %eV
E_out = .33 ;%eV;
num_eig = 9 ;% Number of eigen states to display
saveloc = 'C:\Users\blair\Documents\University\2021\PHY4MES\Computational Lab 2 (2021)-20210504\figures\Q4'; %Save location
ED_eig_plots = 5; % Number of subplots of electron density, for some the density will be so low there's not much point plotting


QW3( a, mu, Temp,Nout, Nin, Nw1, Nw2, E_m, E_out, num_eig, saveloc, ED_eig_plots);


function [W,prob, E,nx_sum, U1, Ec, XX] = QW3(a, mu, Temp,Nout, Nin, Nw1, Nw2, E_m, E_out, num_eig, saveloc, ED_eig_plots)
...
% shift xx so that interface falls at origin for all dopant conc
    ...
    plot(XX,U1,'DisplayName', 'Potential','LineWidth',2)

    title('Potential Profile ','interpreter','latex')
    xlabel(' Position($n m$)','interpreter','latex')
    ylabel('Energy eV','interpreter','latex')
    ...
    figure(6)
    hold on
    for x = 1:num_eig %EDx(x)
        label = strcat('Eigenstate ', num2str(x));
        plot(XX, q*NX(x),'DisplayName',label,'LineWidth',2)
    end
    ...
    legend('Location','best' )
    saveas(gcf,strcat(folder,'charge_density.png'))
    ...
    xlabel(' Position($n m$)','interpreter','latex')
    ylabel('Number of Electrons','interpreter','latex')
    ...
    hold off
    %Below returns the value in eV for the lowest energy eigenvalue
    fprintf('The lowest energy eigenvalue is %d eV.\n',min(D)) 
